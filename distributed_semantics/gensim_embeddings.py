...
def compare_to_gold(gold, taxonomy, model,  model_poincare = None, outliers = [], add_back = False, threshold_add = 0.4, new_nodes = [], log = "", write_file = ""):
    ...
    print(str(recall).replace(".", ',') +'\t' + str(precision).replace(".", ',') + '\t' + str(2*precision *recall / (precision + recall)).replace(".", ',') + '\t' +  str(len(outliers)))
    if log != None:
    ...
    for key in structure:
        ...
        for child in cleaned_co_hyponyms_copy:
            ...
            try:
                ...
                #         #print(child, data_base_word_name)
                #         index_parent_c = model_poincare.kv.rank(node_sense, parent_sense)
                #         #ndex_parent_c = 100000000000000000000000
                #         #print(index_parent_c)
