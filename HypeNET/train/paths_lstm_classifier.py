...
class PathLSTMClassifier(BaseEstimator):
    ...
    def predict(self, X_test, x_y_vectors=None, full_information=False):
        """
        ...
        for chunk in xrange(0, len(X_test), 100):
            ...
            for i, path_set in enumerate(X_test[chunk:chunk + 100]):
                x_y_vector = x_y_vectors[chunk + i] if x_y_vectors is not None else None

                val_one_instance = process_one_instance(builder,
                                                        model, model_parameters,
                                                        path_set, path_cache,
                                                        False,
                                                        dropout=0.0,
                                                        x_y_vectors=x_y_vector
                                                        )
...
def train(builder, model, model_parameters, X_train, y_train, nepochs, alpha=0.01, update=True, dropout=0.0,
    ...
    :param model_parameters: the model parameters
    :param X_train: the train_ instances
    :param y_train: the train_ labels
