...
awk -F$'\t' '{a[$1]++; if (a[$1] == 1) print $1}' $directory/wiki_parsed > $directory/left & PIDLEFT=$!
awk -F$'\t' '{a[$2]++; if (a[$2] == 1) print $2}' $directory/wiki_parsed > $directory/right & PIDRIGHT=$!
wait $PIDLEFT
wait $PIDRIGHT
