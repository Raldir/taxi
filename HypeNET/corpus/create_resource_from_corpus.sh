...
#split -nl/4 $wiki_dump_file $wiki_dump_file"_";
...
for x in {a..d}
do
( awk -v OFS='\t' '{i[$3]++} END{for(x in i){print x, i[x]}}' "$(echo $wiki_dump_file)_a"$x"_parsed" > paths"_a"$x ) &
done
wait
...
for x in {a..d}
do
( python create_resource_from_corpus_2.py "$(echo $wiki_dump_file)_a"$x"_parsed" $resource_prefix ) &
done
wait
...
for x in {a..d}
do
( awk -v OFS='\t' '{i[$0]++} END{for(x in i){print x, i[x]}}' "$(echo $wiki_dump_file)_a"$x"_parsed_id" > id_triplet_file"_a"$x ) &
done
wait
...
rm id_triplet_file_temp id_triplet_file_*;
